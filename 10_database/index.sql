...
-- group by(같은 그룹끼리 몪어서 확인 가능);
SELECT specialize from user GROUP BY speicalize;
SELECT specialize, count(*) from user GROUP BY speicalize;
SELECT specialize, count(*) from user where gender="여" GROUP BY specialize HAVING count(specialize)>=2;
