...
function generateUniqueId() {
  // 타임스템프
  const timestamp = Date.now().toString(36); // toString(36) 36진수로 반환한 문자열

  // 랜덤 문자열
  // "abcdef".substring(2); // 2번 index 부터 끝 index까지 반환
...
wsServer.on("connection", (socket) => {
  ...
  socket.on("message", (message) => {
    console.log(`${message}`); // 버터 객체
    /*
    toString() 메소드를 사용하지 않아도
    `${버퍼객체}` 처럼 템플릿 리터럴을 함께 쓰면 암시적으로 toString() 사용할 수 있음
    */
